...
var azazieColors = [
  {'name': 'lemon', 'hex': 'fef984'},
  {'name': 'peacock', 'hex': '035d69'},
  {'name': 'marigold', 'hex': 'fec559'},
  {'name': 'azalea', 'hex': 'fd73af'},
  {'name': 'ivory', 'hex': 'fefef7'},
  {'name': 'grape', 'hex': '561f58'},
  {'name': 'spa', 'hex': '5bf7ea'},
  {'name': 'dusty rose', 'hex': 'e1bbb8'},
  {'name': 'candy pink', 'hex': 'ffd4e7'},
  {'name': 'moss', 'hex': '4e722d'},
  {'name': 'blushing pink', 'hex': 'fee5ec'},
  {'name': 'burgundy', 'hex': '8b1427'},
  {'name': 'red', 'hex': 'c00017'},
  {'name': 'watermelon', 'hex': 'fa7a79'},
  {'name': 'orange', 'hex': 'ffa147'},
  {'name': 'daffodil', 'hex': 'ffffcd'},
  {'name': 'sage', 'hex': 'e1f9bb'},
  {'name': 'lime green', 'hex': 'dbf647'},
  {'name': 'clover', 'hex': 'aeb953'},
  {'name': 'jade', 'hex': '0baab0'},
  {'name': 'dark green', 'hex': '0a574d'},
  {'name': 'ink blue', 'hex': '174c78'},
  {'name': 'dark navy', 'hex': '1e2c5b'},
  {'name': 'royal blue', 'hex': '212eb1'},
  {'name': 'ocean blue', 'hex': '2478ec'},
  {'name': 'pool', 'hex': '60e0f9'},
  {'name': 'sky blue', 'hex': 'cbeafe'},
  {'name': 'lavender', 'hex': 'd6dcfe'},
  {'name': 'lilac', 'hex': 'e6cbf8'},
  {'name': 'pearl pink', 'hex': 'fcdbd4'},
  {'name': 'fuchsia', 'hex': 'e236a4'},
  {'name': 'regency', 'hex': '4c1992'},
  {'name': 'champagne', 'hex': 'e2d0a8'},
  {'name': 'gold', 'hex': 'ddb766'},
  {'name': 'brown', 'hex': '8f6b53'},
  {'name': 'chocolate', 'hex': '503228'},
  {'name': 'white', 'hex': 'fff'},
  {'name': 'silver', 'hex': 'cfd1cc'},
  {'name': 'black', 'hex': '2d3029'},
  {'name': 'tangerine', 'hex': 'fd9850'},
  {'name': 'turquoise', 'hex': '90f5da'},
  {'name': 'coral', 'hex': 'f9c4b6'},
  {'name': 'emerald', 'hex': '057d40'},
  {'name': 'steel grey', 'hex': '6f7478'},
  {'name': 'wisteria', 'hex': 'c3a5bb'},
  {'name': 'taupe', 'hex': 'd4cdc3'},
  {'name': 'stormy', 'hex': '636784'},
  {'name': 'mint green', 'hex': 'd0ffe5'},
  {'name': 'tiffany blue', 'hex': '3ceadc'},
  {'name': 'mist', 'hex': 'd5e7f5'},
  {'name': 'papaya', 'hex': 'fa9178'},
  {'name': 'mulberry', 'hex': '9a375a'},
  {'name': 'frost', 'hex': 'ebebe7'},
  {'name': 'dusk', 'hex': 'ccbdca'},
  {'name': 'rust', 'hex': 'b53c2d'},
  {'name': 'flamingo', 'hex': 'fca4ae'},
  {'name': 'peach', 'hex': 'ffe0c7'},
  {'name': 'orchid', 'hex': '961d78'},
  {'name': 'tahiti', 'hex': '9e81b6'}
];
  ...
  var showColorsForImage = function($image, $imageSection ) {
    ...
    var matchColor = closestMatch(color);
    var url = matchColor.toLowerCase().replace(' ', '-');
    var newPalette = [];

    for (var p = 0; p < palette.length; p++) {
      var ob = {};
      ob['matchPalette'] = closestMatch(palette[p]);
      ob['urls'] = ob['matchPalette'].toLowerCase().replace(' ', '-');
      ob['colors'] = palette[p];
      newPalette.push(ob);
    }
  ...
  function closestMatch(color) {
    var minDistance = distance(hexStringToByte(azazieColors[0]['hex']), color);
    var minColor = 0;

    for (var i = 1; i < azazieColors.length; i++) {
      var d = distance(hexStringToByte(azazieColors[i]['hex']), color);
      if (d < minDistance) {
        minDistance = d;
        minColor = i;
      }
    ...
    return azazieColors[minColor]['name'].toUpperCase();
  }
...
function hslToXyz(hsl) {
  var x = hsl[1] * Math.cos(hsl[0] * Math.PI);
  var y = hsl[1] * Math.sin(hsl[0] * Math.PI);
  return [x, y, hsl[2]];
}
